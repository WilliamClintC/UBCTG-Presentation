...
Ce résultat peut donc être affecté à une variable via l'opérateur `<-` (ou `=`)[^content-11-1].

[^content-11-1]: Pour des questions de clarté et lisibilité du code, l'affectation globale d'un `if ... else ...` est recommandé lorsque l'expression tient sur une seule ligne.
...
{{< iconify fa6-brands r-project >}} autorise cependant l'usage d'un vecteur de booléen d'une longueur supérieure à 1[^content-11-2].

[^content-11-2]: Depuis {{< iconify fa6-brands r-project >}} `4.2.0`, ce n'est plus vrai.
